...
samples_df = pd.read_csv('samples-dotnet.csv', delimiter=';', header=None, names=['sample_name', 'github_address'])
...
data_folder_path = os.path.join(os.path.expanduser('~'), 'Documents', 'Mestrado', '2s2024', 'Paper JSS', 'SonarQube-analysis', 'data')
...
def create_sonarqube_project(sample_name, lang=None):
    ...
    data = {
        'name': f'{lang}---{sample_name}',
        'project': f'{sample_name}'
    }
...
def clone_repository(github_address):
    ...
    os.system(f'git clone {github_address} {samples_folder}/{github_address.split("/")[-1].replace(".git", "")}')
...
def run_sonar_scanner_dotnet(sample_name, commit_hash, is_latest_commit=False):
    subprocess.run(f'dotnet-sonarscanner begin /k:"{sample_name}" /d:sonar.host.url="http://localhost:9000" /d:sonar.login="sqa_8b5b36d0d8f38e528b7e7535a2708229f50fbc21" /v:"{commit_hash}"', shell=True, check=False)
    result = subprocess.run('dotnet build', shell=True, check=False, capture_output=True, text=True)
    if result.returncode != 0:
        ...
        failed_df.to_csv(f'{data_folder_path}/failed_builds/{sample_name}_{commit_hash}failed_builds.csv', mode='a', header=False, index=False)
        print(f'Failed to build {sample_name} at commit {commit_hash}')
    subprocess.run('dotnet-sonarscanner end /d:sonar.login="sqa_8b5b36d0d8f38e528b7e7535a2708229f50fbc21"', shell=True, check=False)
        ...
        for quality in impact_software_qualities:
            ...
            if total_quality_issues <= 500:
                ...
                for page in range(1, total_pages + 1):
                    ...
                    issues = response.json().get('issues', [])
                    issues_df = pd.DataFrame(issues)
                    issues_df.to_csv(f'data/issues/{sample_name}_{quality}_{page}_issues.csv')
            else:
...
def is_dotnet_project(project_path):
    return any(glob.glob(os.path.join(project_path, '*.csproj'))) or any(glob.glob(os.path.join(project_path, '*.sln')))
...
def run_sonarqube_snippets_part():
    ...
    for issue_key in issues_df['key']:
        try:
            snippets_df = pd.read_csv(f'data/code_snippets/{issue_key}_code_snippets.csv')
    ...
    if snippets_list:
        ...
        merged_snippets_df.to_csv('data/code_snippets/0all_code_snippets.csv', index=False)
    else:
